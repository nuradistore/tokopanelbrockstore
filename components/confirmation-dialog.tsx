...
export function ConfirmationDialog({ open, onOpenChange, planId, onConfirm, isLoading }: ConfirmationDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md glass border-glow text-white rounded-2xl">
        <DialogHeader>
        ...
        <div className="space-y-4">
          <div className="glass-soft p-4 rounded-2xl border border-white/10">
        ...
        <DialogFooter className="flex flex-col sm:flex-row sm:justify-between gap-2">
          ...
          <Button onClick={onConfirm} className="w-full sm:w-auto" disabled={isLoading}>
            {isLoading ? (
              <>
