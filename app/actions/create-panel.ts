...
export async function createPanel(data: PanelData) {
  try {
    ...
    const userResponse = await pterodactyl.createUser(username, email, password, plan.access === "admin")
    ...
    if (plan.access !== "admin") {
      ...
      console.log("Admin plan selected: skipping server creation")
    }
